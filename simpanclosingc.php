...
if (isset($_POST['nextc'])) {
    ...
    header('location: countdownc.php');
    exit();
    ...
    $perusahaan = "Perusahaan C"; // Nama perusahaan
    $_SESSION["perusahaan"] = $perusahaan;

    // Perform the database insertion for tb_perusahaan_c
    $queryPerusahaanC = "INSERT INTO tb_perusahaan_c (mobil, tanggalstartrental, waktustartrental, tanggalendrental, waktuendrental, nama, nomorhp, email, pengambilan, lokasi_lain, harga, perusahaan) 
    ...
    if (mysqli_query($conn, $queryPerusahaanC) && mysqli_query($conn, $queryDashboard)) {
        // Data successfully inserted into both tables

        // Redirect to the success page or any other desired page
        header('Location: cetakstrukc.php');
        exit();
    } else {
