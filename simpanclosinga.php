...
if (isset($_POST['nexta'])) {
    ...
    header('location: countdowna.php');
    exit();
    ...
    $perusahaan = "Perusahaan A"; // Nama perusahaan
    $_SESSION["perusahaan"] = $perusahaan;

    // Perform the database insertion for tb_perusahaan_a
    $queryPerusahaanA = "INSERT INTO tb_perusahaan_a (mobil, tanggalstartrental, waktustartrental, tanggalendrental, waktuendrental, nama, nomorhp, email, pengambilan, lokasi_lain, harga, perusahaan) 
    ...
    if (mysqli_query($conn, $queryPerusahaanA) && mysqli_query($conn, $queryDashboard)) {
        // Data successfully inserted into both tables

        // Redirect to the success page or any other desired page
        header('Location: cetakstruka.php');
        exit();
    } else {
